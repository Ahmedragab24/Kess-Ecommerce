...
function Perfumes() {
  const [perfumes, setPerfumes] = useState<Store[]>([]);
  ...
  useEffect(() => {
    try {
      ...
      axios
        ...
        .then((res) => setPerfumes(res.data.msg));
  ...
  // rendering
  const storeList = perfumes.map((store) => {
    return (
      <Circle
        key={store.id}
        type={"CartSmall"}
  ...
  return (
    <section className="stores">
      <div className="title-container">
        <h1 className="store-title">Men's Perfumes</h1>
