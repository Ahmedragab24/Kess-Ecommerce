...
function VariousClothes() {
  const [variousClothes, setVariousClothes] = useState<Store[]>([]);
  ...
  useEffect(() => {
    try {
      ...
      axios
        .get(
          "http://endlestone.com/kees/APIs/stores/getStores.php?verified=1&categoryID=4"
        )
        .then((res) => setVariousClothes(res.data.msg));
  ...
  // rendering
  const storeList = variousClothes.map((store) => {
    return (
      <Circle
        key={store.id}
        type={"CartSmall"}
  ...
  return (
    <section className="stores">
      <div className="title-container">
        <h1 className="store-title">Various Clothes</h1>
