...
function WomenShoes() {
  const [womenShoes, setWomenShoes] = useState<Store[]>([]);
  ...
  useEffect(() => {
    try {
      ...
      axios
        ...
        .then((res) => setWomenShoes(res.data.msg));
  ...
  // rendering
  const storeList = womenShoes.map((store) => {
    return (
      <Circle
        key={store.id}
        type={"CartSmall"}
  ...
  return (
    <section className="stores">
      <div className="title-container">
        <h1 className="store-title">Women's Shoes</h1>
