...
function WomenSport() {
  const [womenSport, setWomenSport] = useState<Store[]>([]);
  ...
  useEffect(() => {
    try {
      ...
      axios
        ...
        .then((res) => setWomenSport(res.data.msg));
  ...
  // rendering
  const storeList = womenSport.map((store) => {
    return (
      <Circle
        key={store.id}
        type={"CartSmall"}
  ...
  return (
    <section className="stores">
      <div className="title-container">
        <h1 className="store-title">Women's Sport</h1>
