...
function WomenPerfumes() {
  const [womenPerfumes, setWomenPerfumes] = useState<Store[]>([]);
  ...
  useEffect(() => {
    try {
      ...
      axios
        ...
        .then((res) => setWomenPerfumes(res.data.msg));
  ...
  // rendering
  const storeList = womenPerfumes.map((store) => {
    return (
      <Circle
        key={store.id}
        type={"CartSmall"}
  ...
  return (
    <section className="stores">
      <div className="title-container">
        <h1 className="store-title">Women's Perfumes</h1>
