...
if (isset($_POST['submit'])) {
    ...
    $sql = "UPDATE products SET name=?, name_ar=?, description=?, description_ar=?, store_id=?, photo=? WHERE id=?";
    $stmt = $conPDO->prepare($sql);
    $res = $stmt->execute([$name, $name_ar, $description, $description_ar, $store_id, $newPhotoName, $id]);
    ...
    <section class="mt-5 p-2">
        <div class="container my-5">
            <form method="POST" class="cent" enctype="multipart/form-data">
                ...
                <div class="mb-3">
                    <label class="form-label" for="store_id">Main Category</label>
                    <select name="store_id" id="store_id" class="form-control">
                        <?php
                        $sql1 = "SELECT * FROM stores";
                        $result1 = $conPDO->query($sql1);
                        $count = $result1->rowCount();
                        if ($count > 0) {
                            while ($row1 = $result1->fetch(PDO::FETCH_ASSOC)) {
                                $selected = ($row1['id'] == $row['store_id']) ? 'selected' : '';
                                ?>
